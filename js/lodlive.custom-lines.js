(function($) {
	...
	var methods = {
		...
		isPartOfLine : function(label, x1, y1, x2, y2, canvas, toId) {
			// eseguo i calcoli e scrivo la riga di connessione tra i cerchi

			var lineangle = (Math.atan2(y2 - y1, x2 - x1) * 180 / Math.PI) + 180;
			var x2bis = x1 - Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));

			var xM = x1 + (x2bis - x1) / 2;
			var yM = y1 +  (x2bis - x1) / 2;
			console.info("x1 " + x1 + " -- x2bis " + x2bis)
			canvas.rotateCanvas({
				angle : lineangle,
				x : x1,
				y : y1
			...
			/*			var lineangle = (Math.atan2(y2 - y1, x2 - x1) * 180 / Math.PI) + 180;
			 ...
			 console.info("lineangle " + lineangle);
			 console.info("x1 " + x1);
			 console.info("y1 " + y1);
			 console.info("x2bis " + x2bis);
...
*/
			 console.info("x2bis +x1/2 " + (x2bis + x1) / 2);
			 console.info("y1 +y1 " + (y1 + y1) / 2);
			 ...
			 console.info("x "+((x2bis + x1) / 2));
			 console.info("y "+((y1 + y1 - (x1 > x2 ? 18 : -18)) / 2));
